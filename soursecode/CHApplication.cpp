...
namespace me
{
	Application::Application()
		:mHwnd(nullptr),mHdc(nullptr),mspeed(0),mX(0),mY(0)
	{
	...
	void Application::Render()
	{
		...
		HBRUSH oldbrush = (HBRUSH)SelectObject(mHdc, brush); //파랑 브러쉬 DC에 선택, 흰색 브러쉬 반환
		HPEN redPen = CreatePen(PS_SOLID, 2, RGB(255, 0, 0));   //테두리 만들기
		HPEN oldPen = (HPEN)SelectObject(mHdc, redPen);
		Rectangle(mHdc, 100+mX, 100+mY, 200+mX, 200+mY); //사각형 만드는 함수
